...
#define POWER_MOSFET_PIN 7 // WS2812 consume ~1mA/led when off. By installing a MOSFET it will cut the power to the leds when lights ore off.
#define ledGPIO 4
#define DATA_PIN 10
...
NeoPixelBus<NeoGrbFeature, NeoEsp32Rmt0Ws2812xMethod>* strip = NULL;

void blinkLed(uint8_t count, int interval = 200) {
...
void ChangeNeoPixels(uint16_t newCount) // this set the number of leds of the strip based on web configuration
{
  ...
  strip = new NeoPixelBus<NeoGrbFeature, NeoEsp32Rmt0Ws2812xMethod>(newCount, DATA_PIN); // and recreate with new count
  strip->Begin();
}
