...
uint16_t pixelLight (uint16_t pixel) {
	...
	if (inTransition) {
		if (lightsCount == 1) {
			...
			for (uint16_t pixel = 0; pixel < pixelCount; pixel++) {
				uint16_t light = pixelLight(pixel);
		...
		for (uint8_t light = 0; light < lightsCount; light++) {
			...
			if (light == 0) {
				if (lightsCount == 2) {
					...
					for (uint16_t pixel = 0; pixel < lightLedsCount; pixel++) {
